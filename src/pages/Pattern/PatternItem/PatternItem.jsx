...
export default function PatternItem({ item, i }) {
  ...
  useEffect(() => {
    setWidth(
      -1 * (sliderWidth.current.scrollWidth - sliderWidth.current.offsetWidth)
    );
  }, [sliderWidth]);
  ...
  return (
    <div className={s.item}>
      ...
      <motion.div ref={sliderWidth} className={s.slider}>
        <motion.div
          drag='x'
      ...
      {item.map ? (
        <YMaps>
          <Map className={s.map} defaultState={defaultState} width='100%'>
